        ...
        <div className="flex items-center gap-1.5">
          {isExecutable && (
             <button
               onClick={handleRun}
               disabled={isRunning}
               className="flex items-center gap-1.5 px-2 py-1.5 hover:bg-white/10 rounded-md transition-all active:scale-95 text-emerald-400"
               title="Run Code"
             >
            ...
            <div className="border-t border-[#333] bg-[#1e1e1e]">
              ...
              <div className="p-3 font-mono text-[13px] whitespace-pre-wrap max-h-60 overflow-y-auto">
                ...
                {!executionOutput.stdout && !executionOutput.stderr && executionOutput.code === 0 && (
                   <div className="text-emerald-400 opacity-80 text-xs text-italic">Execution finished with no output.</div>
                )}
              </div>
            </div>
          )}
