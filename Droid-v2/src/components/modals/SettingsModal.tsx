...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-[var(--bg-main)] z-[100] flex flex-col pt-4 animate-in slide-in-from-bottom-5 duration-200">
      <input 
        type="file" 
        ref={avatarInputRef} 
      ...
      <div className="flex items-center px-4 py-2 border-b border-[var(--border-drawer)]">
        <button onClick={onClose} className="p-2 mr-2 active:bg-[var(--surface-hover)] rounded-full transition-colors">
          <X className="w-6 h-6 text-[var(--text-primary)]" />
        </button>
        <h2 className="text-xl font-serif font-bold text-[var(--text-primary)]">Settings</h2>
      </div>
          ...
          <div className="p-6 space-y-8 max-w-2xl mx-auto">
            ...
            <section>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-[var(--text-primary)]">Chat font</span>
                  <select 
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex-1 mr-4">
                    <p className="text-sm font-medium text-[var(--text-primary)]">Response completions</p>
                    <p className="text-[11px] text-[var(--text-muted)]">Get notified when DROIDE has finished a response. Useful for long-running tasks.</p>
                  </div>
                  <button 
                    onClick={() => setSettings(p => ({...p, notifications: {...p.notifications, taskCompletion: !p.notifications.taskCompletion}}))} 
                    className={`w-11 h-6 rounded-full transition-colors relative shrink-0 ${settings.notifications?.taskCompletion ? 'bg-blue-500' : 'bg-gray-300'}`}
                  >
                    <div className={`w-5 h-5 rounded-full bg-white absolute top-0.5 transition-transform ${settings.notifications?.taskCompletion ? 'translate-x-5.5' : 'translate-x-0.5'}`}></div>
                  </button>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex-1 mr-4">
                    <p className="text-sm font-medium text-[var(--text-primary)]">Dispatch messages</p>
                    <p className="text-[11px] text-[var(--text-muted)]">Get a push notification on your phone when DROIDE messages you in Dispatch.</p>
                  </div>
                  <button 
                    onClick={() => setSettings(p => ({...p, notifications: {...p.notifications, dispatchMessages: !p.notifications.dispatchMessages}}))} 
                    className={`w-11 h-6 rounded-full transition-colors relative shrink-0 ${settings.notifications?.dispatchMessages ? 'bg-blue-500' : 'bg-gray-300'}`}
                  >
                    <div className={`w-5 h-5 rounded-full bg-white absolute top-0.5 transition-transform ${settings.notifications?.dispatchMessages ? 'translate-x-5.5' : 'translate-x-0.5'}`}></div>
                  </button>
                </div>
              </div>
            </section>
