...
function useLocalStorage<T>(key: string, initialValue: T) {
  ...
  const setValue = (value: T | ((val: T) => T)) => {
    setStoredValue((prev) => {
      ...
      // Immediately persist to localStorage (no debounce for setValue calls)
      try {
  ...
  // Debounced sync to localStorage (catches streaming updates that bypass direct setValue)
  useEffect(() => {
    ...
    }, 300); // 300ms debounce - balance between performance and data safety
...
export default function App() {
  ...
  const [pendingImages, setPendingImages] = useState<{ mimeType: string, data: string, name?: string, isUrl?: boolean }[]>([]);
  ...
  const clearSessionMessages = (id: string) => {
    if (!id) { showToast('No active session'); return; }
  ...
  // --- Export Logic ---
  const exportToJSON = () => {
    if (!currentSession.id) { showToast('No active chat to export'); return; }
    vibrate();
  ...
  // Ensure currentSession always exists in the sessions array (fallback creates a new tracked session)
  const currentSession = sessions.find(s => s.id === currentSessionId) ?? (() => {
    // If no session found, return a virtual session (executeSend will create a real one)
  ...
  const mapHistory = (msgs: Message[]): ChatHistoryItem[] => {
    return msgs
      .filter(m => !m.isStreaming && m.role !== 'system') // system messages go in systemInstruction, not history
      .map(m => {
        const parts: any[] = [];
        if (m.content) parts.push({ text: m.content });
        // Only include non-URL images (isUrl images are external references, not base64 inline data)
        if (m.images) m.images.forEach(img => {
          if (!( img as any).isUrl) {
            parts.push({ inlineData: { mimeType: img.mimeType, data: img.data } });
          }
        });
        // Ensure parts is never empty (API requirement)
    ...
    try {
      ...
      const handleToolCall = (call: { name: string, args: any }) => {
        ...
        if (call.name === 'update_app_settings') {
          ...
          const { fileName, content: fileContent, mimeType } = call.args;
          const effectiveMime = mimeType || 'text/plain';
          try {
            // For text files, decode from base64 if needed, otherwise use directly
            let blobContent: string | Uint8Array = fileContent;
            ...
            const blob = new Blob([blobContent], { type: effectiveMime });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = fileName;
          ...
          setPendingImages(prev => [...prev, { name: 'Generated Image', data: imageUrl, mimeType: 'image/jpeg', isUrl: true }]);
      ...
      const adaptiveThinkingInstr = settings.adaptiveThinking ? ADAPTIVE_THINKING_INSTRUCTION : '';
      const fullInstruction = settings.systemPrompt + adaptiveThinkingInstr + 
