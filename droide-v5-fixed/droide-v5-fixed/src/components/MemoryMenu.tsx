...
export function MemoryMenu({ settings, setSettings, onClose, vibrate, showToast }: any) {
  ...
  const [activeTab, setActiveTab] = useState<'memory' | 'instructions' | 'writing' | 'formatting' | 'notifications'>('instructions');
  ...
  return (
    <div className="fixed inset-0 bg-[var(--bg-main)] z-[120] flex flex-col pt-2 animate-in fade-in duration-150">
      ...
      <div className="flex px-4 py-2 space-x-2 border-b border-[var(--border-drawer)] overflow-x-auto scrollbar-hide">
        <button onClick={() => setActiveTab('instructions')} className={`px-4 py-2 whitespace-nowrap rounded-lg text-sm font-medium transition-colors ${activeTab === 'instructions' ? 'bg-[var(--accent)] text-white' : 'bg-[var(--surface)] text-[var(--text-muted)]'}`}>
          <SlidersHorizontal className="w-4 h-4 inline-block mr-1.5" /> Custom Instructions
        </button>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3 p-4 bg-[var(--accent)]/10 rounded-xl border border-[var(--accent)]/20 text-[var(--accent)]">
              ...
              <div className="text-sm">
                <p className="font-semibold mb-1">About You & How AI Should Respond</p>
                <p>Provide permanent instructions on who you are, what you do, and how you want the AI to behave or format its responses.</p>
              </div>
            </div>

            <div className="bg-[var(--surface)] rounded-2xl p-4 border border-[var(--border-drawer)]">
              <h3 className="font-medium mb-3">Custom Instructions</h3>
              <textarea
                value={settings.customInstructions || ''}
                onChange={(e) => setSettings({ ...settings, customInstructions: e.target.value })}
                placeholder="E.g., I am a software engineer. Always provide response in concisely with code blocks without explanations."
                className="w-full h-40 bg-[--bg-main] border border-[--border-drawer] rounded-xl px-3 py-3 text-sm focus:outline-none focus:border-[var(--accent)] transition-colors resize-none mb-2 placeholder-[var(--text-muted)]"
              />
              <p className="text-xs text-[var(--text-muted)] flex justify-end">Automatically applied to all new chats.</p>
            </div>
          </div>
        )}
