...
import { X, BookMarked, Plus, Check, Edit2, Trash2, Bot, User, FileText, Sparkles } from 'lucide-react';
...
interface PromptLibraryModalProps {
  ...
  onSetCustomInstructions?: (content: string) => void;
...
export const PromptLibraryModal: React.FC<PromptLibraryModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'personal' | 'subagents' | 'instructions'>('personal');
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
      ...
      >
        <div className="p-5 pb-0 flex flex-col z-10 relative bg-[var(--bg-main)]">
          ...
          <div className="flex space-x-6 border-b border-[var(--border-drawer)] relative">
            ...
            >
              ...
              Personal
              {activeTab === 'personal' && (
                <motion.div layoutId="libTabIndicator" transition={{ duration: 0.15 }} className="absolute bottom-0 left-0 right-0 h-[2px] bg-[var(--text-primary)] rounded-t-full" />
              )}
            </button>
            <button
            ...
            <button
              ...
              className={`pb-3 font-bold text-sm tracking-tight transition-colors relative flex items-center gap-2 ${activeTab === 'instructions' ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-hide pb-safe">
          ...
          {activeTab === 'instructions' && (
            <div className="space-y-6">
              <div className="p-5 bg-[var(--accent)]/5 rounded-3xl border border-[var(--accent)]/20 space-y-2">
                <div className="flex items-center gap-2 text-[var(--accent)] mb-1">
                  <Sparkles className="w-4 h-4" />
                  <h4 className="text-xs font-bold uppercase tracking-wider">Instruction Library</h4>
                </div>
                <p className="text-[13px] text-[var(--text-muted)] leading-relaxed">
                  Apply these global behavioral instructions to customize how the AI responds across all interactions.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                {INSTRUCTION_PRESETS.map(preset => (
                  <motion.div 
                    ...
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="group p-5 bg-[var(--surface)] rounded-3xl border border-[var(--surface-border)] hover:border-[var(--accent)]/30 transition-all shadow-sm"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h5 className="font-bold text-[var(--text-primary)]">{preset.name}</h5>
                    </div>
                    <p className="text-xs text-[var(--text-muted)] mb-4 leading-relaxed">{preset.description}</p>
                    <div className="flex flex-row gap-2">
                      <button 
                      ...
                      <button 
                        onClick={() => {
                          onSetCustomInstructions?.(preset.prompt);
                        }}
                        className="flex-[1.5] py-3 bg-[var(--surface-hover)] text-[var(--text-primary)] border border-[var(--surface-border)] hover:border-[var(--accent)]/50 rounded-2xl text-xs font-bold active:scale-95 transition-all flex items-center justify-center gap-2"
                      >
                        Set as Global Instruction
                      </button>
                    </div>
                  </motion.div>
                ))}
