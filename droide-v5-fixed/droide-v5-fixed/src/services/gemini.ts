...
${systemInstruction || ''}`;
  ...
  let filteredTools = appTools;
  if (config.toolAccessMode !== 'always') {
    const fnToolsItem = appTools.find((t: any) => t.functionDeclarations);
    const allDeclarations: any[] = fnToolsItem ? (fnToolsItem as any).functionDeclarations : [];
    const allowedDeclarations = allDeclarations.filter((d: any) => {
      if (!config.generateMemory && d.name === 'manage_memory') return false;
      if (!config.codeExecution && (d.name === 'execute_code' || d.name === 'save_file' || d.name === 'read_url_content')) return false;
      if (config.codeExecution && !config.networkEgress && d.name === 'read_url_content') return false;
      return true;
    });
    filteredTools = [
      appTools[0], // googleSearch
      { functionDeclarations: allowedDeclarations }
    ];
  }
  if (config.toolAccessMode === 'never') {
    filteredTools = [];
  }
  ...
  if (config.provider) {
    ...
    let currentOpenAIMessages = [...openAIMessages];
    ...
    while (isContinuing) {
      ...
      const requestBody: any = {
        ...
        messages: currentOpenAIMessages,
        stream: true,
        max_tokens: 4096
      };
      ...
      if (filteredTools.length > 0) {
        ...
        filteredTools.forEach((toolItem: any) => {
          if (toolItem.functionDeclarations) {
            toolItem.functionDeclarations.forEach((decl: any) => {
              openAITools.push({
                type: 'function',
                function: {
                  name: decl.name,
      ...
      let buffer = '';
      const inProgressToolCalls: { [index: number]: { id: string, name: string, arguments: string } } = {};
              ...
              if (data.choices && data.choices[0].delta) {
                ...
                if (delta.tool_calls) {
                  delta.tool_calls.forEach((tc: any) => {
                    const idx = tc.index;
                    if (!inProgressToolCalls[idx]) {
                      inProgressToolCalls[idx] = { id: tc.id || `call_${idx}`, name: '', arguments: '' };
                    }
                    if (tc.function?.name) {
                      inProgressToolCalls[idx].name += tc.function.name;
                    }
                    if (tc.function?.arguments) {
                      inProgressToolCalls[idx].arguments += tc.function.arguments;
                    }
                  });
                }
              }
      ...
      const tcKeys = Object.keys(inProgressToolCalls);
      if (tcKeys.length > 0) {
        const toolCalls = tcKeys.map(k => ({
          id: inProgressToolCalls[k as any].id,
          type: 'function',
          function: {
            name: inProgressToolCalls[k as any].name,
            arguments: inProgressToolCalls[k as any].arguments
          }
        }));

        currentOpenAIMessages.push({
          role: 'assistant',
          content: null,
        ...
        for (const tc of toolCalls) {
          let callArgs: any = {};
          try {
            callArgs = tc.function.arguments ? JSON.parse(tc.function.arguments) : {};
          } catch(e) {}

          // Notify App about tool usage (for UI updates like setActiveTool)
          const externalResult = onToolCall ? onToolCall({ name: tc.function.name, args: callArgs }) : undefined;

          let toolResult: any = null;
          if (tc.function.name === 'execute_code') {
            const { language, code } = callArgs;
            toolResult = await executeCodeLocally(language, code);
          } else if (tc.function.name === 'read_url_content') {
            const { url } = callArgs;
            toolResult = await fetchUrlLocally(url);
          } else if (tc.function.name === 'get_weather') {
            const { lat, lon } = callArgs;
            toolResult = await getWeather(lat, lon);
          } else if (tc.function.name === 'search_places') {
            const { query } = callArgs;
            toolResult = await searchPlaces(query);
          } else if (tc.function.name === 'get_sports_data') {
            const { league, query } = callArgs;
          ...
          // Use external result only when no local result available (for App-side tools)
    ...
    while (isContinuing) {
      ...
      isContinuing = false;
      try {
        const streamResponse = await chat.sendMessageStream({ message: currentMessageParts.length > 0 ? currentMessageParts : [{ text: "" }] });
        ...
        for await (const chunk of streamResponse) {
          ...
          if (calls && calls.length > 0) {
            for (const call of calls) {
              // Notify App about tool usage (for UI updates like setActiveTool)
              const externalResult = onToolCall ? onToolCall({ name: call.name, args: call.args }) : undefined;
