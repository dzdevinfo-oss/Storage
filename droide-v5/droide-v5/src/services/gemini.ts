...
${systemInstruction || ''}`;
  ...
  if (config.toolAccessMode !== 'always') {
    const allowedDeclarations = appTools[1].functionDeclarations.filter((d: any) => {
    ...
    while (isContinuing) {
      ...
      if (tcKeys.length > 0) {
        ...
        for (const tc of toolCalls) {
          ...
          if (onToolCall) onToolCall({ name: tc.function.name, args: callArgs });
          ...
          if (onToolCall) {
            const externalResult = onToolCall({ name: tc.function.name, args: callArgs });
