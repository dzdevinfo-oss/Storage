...
export function ArtifactViewer({ code, language }: ArtifactViewerProps) {
  ...
  const generateSrcDoc = () => {
    if (isHtml) {
      ...
      return `<!DOCTYPE html><html><head><script src="https://cdn.tailwindcss.com"></script></head><body class="p-4 font-sans">${code}</body></html>`;
    }
    if (isDiagram) {
      return `<!DOCTYPE html><html><head><script type="module">import mermaid from 'https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.esm.min.mjs'; mermaid.initialize({startOnLoad:true, theme: 'base'});</script></head><body class="flex items-center justify-center min-h-screen p-4 m-0 bg-white"><pre class="mermaid">${code}</pre></body></html>`;
    }
    if (language === 'javascript' || language === 'js') {
       return `<!DOCTYPE html><html><head><script src="https://cdn.tailwindcss.com"></script></head><body class="p-4"><div id="root"></div><script>${code}</script></body></html>`;
    }
    return code;
  };
